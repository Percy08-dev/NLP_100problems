...
# 共起頻度を求める.
# 共起は同文中に登場した場合を求める. 
def Co_word_counter(data:List[List[dict]], target:str) -> list:
    # 出現数はkey = word, value = countの形で辞書型に保存する. 
    res = dict()

    for sentence in data:
        # targetが含まれていない場合, その文を共起頻度の算出から除く. 
        if not(target in [i["surface"] for i in sentence]):
            continue
    ...
    # targetとの共起頻度なので, targetは含まない. 削除する. 
    del res[target]
...
def main():
    ...
    word_counts = Co_word_counter(data, "猫")# 単語の出現数を算出
